...
def get_fmm_num_parameters(n_leads: int, n_waves: int = 5) -> Tuple[int, int]:
    """Return the total number of parameters and the per-wave count (A, alpha, beta, omega)."""
...
def get_coeff_indexes(coeff: str, wave_idx: int, n_leads: int, n_waves: int = 5) -> Tuple[int, int]:
    """Compute the start and end indexes for a given coefficient."""
...
def get_M_indexes(n_leads: int, n_waves: int = 5) -> Tuple[int, int]:
    """Get the indexes of M at the end of the array."""
...
def get_circular_mask(n_leads: int, n_waves: int = 5) -> np.ndarray:
    """Boolean mask for the alpha and beta indexes."""
...
def convert_fmm_dict_to_array(fmm: Dict[str, Dict[str, np.ndarray]],
    ...
    """Convert a nested FMM dict into a flat array."""
...
def array_to_fmm_dict(coeffs: np.ndarray, num_leads: int, num_waves: int = 5) -> Dict[str, Dict[str, np.ndarray]]:
    """Convert a flat coefficient array into a nested FMM dict."""
    ...
    for i, wave in enumerate(DEFAULT_WAVES[:num_waves]):
        ...
        for field in ('A', 'alpha', 'beta', 'omega'):
            ...
            if field in ('alpha', 'omega'):
                # replicate the scalar value per lead
...
def extract_lead_coeffs(arr: np.ndarray, lead: int, n_leads: int, n_waves: int = 5) -> np.ndarray:
    """Extract the FMM parameters of a single lead.

    Iterates over every wave and every coefficient, so all n_waves*4 parameters
    (plus the offset M) are copied.

    A and beta are stored per lead (n_leads values each), so the requested lead is
    offset into the block; alpha and omega are shared scalars per wave.
    """
    ...
    for wave_idx in range(n_waves):
        for coeff in ('A', 'alpha', 'beta', 'omega'):
            s_all, _ = get_coeff_indexes(coeff, wave_idx, n_leads, n_waves)
            s_one, _ = get_coeff_indexes(coeff, wave_idx, 1, n_waves)
...
def angle_to_cos_sin(X: np.ndarray, ang_mask: np.ndarray, zero_one: bool = False) -> np.ndarray:
    """Append the cosine and sine of the angles to the coefficient array."""
...
def sort_fmm_coeffs_array(fmm_array: np.ndarray, n_leads: int, n_waves: int = 5) -> np.ndarray:
    """Sort the FMM coefficients by increasing alpha angle."""

    # linearize the alpha angles
