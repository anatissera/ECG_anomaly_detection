...
def plot_confusion_matrix(cm: np.ndarray, class_names: list = ["Negative", "Positive"], title: str = "Confusion Matrix", cmap: str = "Blues", figsize: tuple = (6, 6), fontsize: int = 16, numberfmt: str = "d"):
    ...
    ax.set_xlabel("Prediction", fontsize=fontsize, labelpad=9)
    ax.set_ylabel("True Label", fontsize=fontsize, labelpad=9)
...
def plot_roc_pr(y_true: np.ndarray, y_score: np.ndarray, figsize: tuple[float, float] = (6, 4), lw: float = 3, font_size: int = 14, roc_color: str = 'teal', pr_color: str = 'orchid') -> None:
    ...
    plt.title("Precision-Recall Curve", fontsize=font_size + 2)
    plt.xticks(fontsize=font_size - 2)
    plt.yticks(fontsize=font_size - 2)
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def plot_training_losses(train_recon_losses: list[float], train_kl_losses: list[float], train_total_losses: list[float], figsize: tuple[float, float] = (6, 4), lw: float = 2.5, font_size: int = 14, recon_color: str = 'salmon', kl_color: str = 'seagreen', total_color: str = 'cornflowerblue') -> None:
    """Show Recon, KL and Total losses per epoch."""
    ...
    plt.title('Loss Evolution during Training', fontsize=font_size + 2, pad=12)
