...
def subplot_ecg(ecg_data: np.ndarray, labels: Optional[Sequence[int]] = None, num_to_plot: int = 9, lead: int = 0, indices: Optional[Sequence[int]] = None):
    """Show a grid of ECG signal subplots."""

    grid_size = int(np.sqrt(num_to_plot))
    assert grid_size**2 == num_to_plot, "num_to_plot must be a perfect square"

    if indices is None:
        indices = np.random.choice(len(ecg_data), num_to_plot, replace=False)
    else:
        assert len(indices) == num_to_plot, "The number of indices must match num_to_plot"
    ...
    for i, idx in enumerate(indices, start=1):
        ...
        title = f"Idx {idx}: {'Anomalous' if clase else 'Normal'}" if labels is not None else f"Idx {idx}"
        ax.set_title(title)
...
def plot_ecg_with_fmm(ecg: np.ndarray, coeffs: np.ndarray, convert_fn, num_leads: int, seq_len: int, fs: int = 100, lead: int = 0, label: str = None):
    """Draw the raw ECG signal and its FMM reconstruction."""

    t = np.arange(seq_len) / fs

    # Original signal
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(t, ecg, color=MAIN_COLOR, linewidth=THICK_LINE, label=label or 'Original ECG')
    ax.set_xlabel('Time (s)', fontsize=FONT_SIZES['labels'])
    ax.set_ylabel('Amplitude', fontsize=FONT_SIZES['labels'])
    ax.tick_params(axis='both', labelsize=FONT_SIZES['ticks'])
    ax.set_xlim(0, 1.1)

    title = label or 'Raw ECG'
    ...
    reconstruction = params['P']['M'][lead] + sum(w for _, w in waves)

    fig, ax = plt.subplots(figsize=(9, 4.5))
    ax.plot(t, reconstruction, color=MAIN_COLOR, linewidth=THICK_LINE, label='FMM reconstruction')
    for name, wave in waves:
        ax.plot(t, wave, linestyle='--', color=WAVE_COLORS[name], linewidth=2, label=f'{name} wave')

    ax.set_xlabel('Time (s)', fontsize=FONT_SIZES['labels'])
    ax.set_ylabel('Amplitude', fontsize=FONT_SIZES['labels'])
    ax.tick_params(axis='both', labelsize=FONT_SIZES['ticks'])
    ax.set_title('FMM reconstruction with waves', fontsize=FONT_SIZES['title'], fontweight='bold')
    ax.title.set_bbox(dict(facecolor=TITLE_BG, edgecolor='none', boxstyle='round,pad=0.5', alpha=0.7))
    ax.legend(fontsize=FONT_SIZES['legend'])
    ax.grid(alpha=0.2)
    plt.tight_layout()
...
def _generate_wave_segment(wave_dict: Dict[str, np.ndarray], seq_len: int, fs: int, lead: int) -> np.ndarray:
    """Generate the FMM wave segment for a single lead."""
