...
_RECORD_RE = re.compile(r'sample_(\d+)_beat')


def _record_of(fname: str) -> int:
    """Recording number a beat file belongs to ('sample_12_beat_3' -> 12)."""
    m = _RECORD_RE.search(fname)
    if m is None:
        raise ValueError(f"cannot parse a record number out of {fname!r}")
...
def _load_fmm_split(base: str, split: str, sequence_length: int, n_leads: int, n_waves: int, lead: int, delete_high_A: bool, url_id: str) -> Dict:
    """
    Internal loader for one FMM split.

    Args:
        base: Base path of the dataset.
        split: Name of the subdirectory ("train", "test" or "all").
        sequence_length: Fixed length of the data sequence.
        n_leads: Number of leads to use.
        n_waves: Number of waves in the FMM model.
        lead: Lead index to extract when reducing from multi-lead.
        delete_high_A: If True, filter out samples with high amplitudes.
        url_id: Google Drive ID used for download.
    Returns:
        Dictionary with keys: 'data', 'labels', 'coefficients', 'sizes', 'records'.

    Each file holds one heartbeat and is named ``sample_<record>_beat_<n>``, so a
    single ECG recording contributes several beats. The record number is kept in
    'records' so that splits can be grouped by recording: splitting beats at
    random would put beats of the same patient on both sides of the split.
    """
    ...
    out['records'] = np.array([_record_of(f) for f in files], dtype=np.int64)
...
def get_ptb_xl_fmm_dataset(datapath: str = './data', frequency: int = 100, lead: int = 0, delete_high_A: bool = True, num_leads: int = 12, num_waves: int = 5, sequence_length: int = 100, **kwargs) -> Dict:
    """
    Load and preprocess the PTB-XL dataset with FMM models.

    Args:
        datapath: Root data folder.
        frequency: Sampling frequency (not used internally).
        lead: Lead index to use when num_leads=1.
        delete_high_A: Filter out high amplitudes if True.
        num_leads: Number of leads to keep.
        num_waves: Number of FMM waves.
        sequence_length: Uniform sequence length.
    Returns:
        Dictionary with keys 'train', 'test' and 'params'.
    """
...
def get_shaoxing_fmm_dataset(datapath: str = './data', frequency: int = 500, lead: int = 0, test_size: float = 0.2, split_seed: int = None, delete_high_A: bool = False, num_leads: int = 12, num_waves: int = 5, sequence_length: int = 100, **kwargs) -> Dict:
    """
    Load and split the Chapman-Shaoxing dataset with FMM.

    Args:
        datapath: Root data folder.
        frequency: Original sampling frequency.
        lead: Lead to extract when num_leads=1.
        test_size: Test fraction for the split.
        split_seed: Random seed for reproducibility.
        delete_high_A: Filter out coefficients with high A if True.
        num_leads: Number of leads kept.
        num_waves: Number of FMM waves.
        sequence_length: Sequence length.
    Returns:
        Dictionary with 'train', 'test' and 'params'.

    The train/test split is grouped by recording, so all the beats of a given
    recording end up on the same side. Splitting beats at random would leave
    beats of the same patient in both train and test and inflate the metrics.
    """
    ...
    C, S, G = ds_all['coefficients'], ds_all['sizes'], ds_all['records']
    ...
    C_tr, C_te = C[tr_idx], C[te_idx]
    S_tr, S_te = S[tr_idx], S[te_idx]
    G_tr, G_te = G[tr_idx], G[te_idx]
    assert not (set(G_tr) & set(G_te)), "recording leaked across the Chapman split"
